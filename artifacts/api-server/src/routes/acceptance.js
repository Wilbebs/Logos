...
router.post('/:id/acceptance/send', async (req, res) => {
  try {
    ...
    const _resend = getResend(); if (!_resend) return res.status(503).json({ error: "RESEND_API_KEY not configured" }); const { data: sendData, error: sendError } = await _resend.emails.send({
      from: fromAddress,
      to: recipientEmail,
      subject,
      text: fullEmailText,
